...
if __name__=="__main__":
    ...
    to generate 3D segmentation. This example is more involved because we need to do nonstandard preprocessing to improve contrast and edge information to be suitable for Cellpose application.
    ...
    plt.subplot(131)
    plt.title('Chan 0 Max. Projection')
    plt.imshow(img[0,:].max(axis=0), cmap='gray')
    plt.subplot(132)
    plt.imshow(img[0,:].max(axis=1), cmap='gray')
    plt.subplot(133)
    plt.imshow(img[0,:].max(axis=2), cmap='gray')
    plt.savefig(os.path.join(savecellfolder,'chan0_max-projection.png'), dpi=300, bbox_inches='tight')
    
    
    plt.figure(figsize=(20,20))
    plt.subplot(131)
    plt.title('Chan 1 Max. Projection')
    plt.imshow(img[1,:].max(axis=0), cmap='gray')
    plt.subplot(132)
    plt.imshow(img[1,:].max(axis=1), cmap='gray')
    plt.subplot(133)
    plt.imshow(img[1,:].max(axis=2), cmap='gray')
    plt.savefig(os.path.join(savecellfolder, 'chan1_image_max-projection.png'), dpi=300, bbox_inches='tight')
    ...
    uSegment3D_fio.save_segmentation(os.path.join(savecellfolder,
                                                'uSegment3D_labels.tif'), segmentation3D)
    ...
    uSegment3D_fio.write_pickle(os.path.join(savecellfolder,
                                                'uSegment3D_3Dcombined_probs_and_gradients'), 
    ...
    print("This is a fairly hacky way of fixing the issue...")
    ...
    # save this out 
    uSegment3D_fio.save_segmentation(os.path.join(savecellfolder,
                                                  'uSegment3D_labels_postprocess.tif'), segmentation3D_filt)
    ...
    uSegment3D_fio.save_segmentation(os.path.join(savecellfolder,
                                                  'uSegment3D_labels_postprocess-diffuse.tif'), segmentation3D_filt_diffuse)
    
    
    print("The below bit is currently failing...")
    ...
    uSegment3D_fio.save_segmentation(os.path.join(savecellfolder,
                                                  'uSegment3D_labels_postprocess-diffuse-guided_filter.tif'), segmentation3D_filt_guide)
